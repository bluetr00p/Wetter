        ...
        Alamofire.request(RequestURL).responseJSON { response in
            if let json = response.result.value {
                ...
                if !testDayType.contains("night") {
                    let temps = "Temp: " + Temps[2].stringValue + "/" + Temps[3].stringValue + "\n"
                    let day = JSONdecoded["time"]["startPeriodName"][2].stringValue + "\n"
                    let desc = JSONdecoded["data"]["text"][2].stringValue
