        ...
        Alamofire.request(RequestURL).responseJSON { response in
            if let json = response.result.value {
                ...
                if !testDayType.contains("night") {
                    let temps = "Temp: " + Temps[6].stringValue + "/" + Temps[7].stringValue + "\n"
                    let day = JSONdecoded["time"]["startPeriodName"][6].stringValue + "\n"
                    let desc = JSONdecoded["data"]["text"][6].stringValue
                    self.miniDesc.text = day + temps + desc
                    
                    let imageUrlString = JSONdecoded["data"]["iconLink"][6].stringValue
                ...
                if testDayType.contains("night") {
                    let temps = "Temp: " + Temps[5].stringValue + "/" + Temps[6].stringValue + "\n"
                    let day = JSONdecoded["time"]["startPeriodName"][5].stringValue + "\n"
                    let desc = JSONdecoded["data"]["text"][5].stringValue
                    self.miniDesc.text = day + temps + desc
                    
                    let imageUrlString = JSONdecoded["data"]["iconLink"][5].stringValue
                    let httpsed = imageUrlString.replacingOccurrences(of: "http://", with: "https://")
                    let imageUrl = URL(string: httpsed)!
                    let imageData = try! Data(contentsOf: imageUrl)
                    let image = UIImage(data: imageData)
